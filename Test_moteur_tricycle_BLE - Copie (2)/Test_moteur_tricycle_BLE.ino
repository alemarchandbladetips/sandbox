...
#define REAR_VS_FRONT_GAIN 0.3f 
// Vitesse différentielle max par rapport à la vitesse avant
#define LAT_VS_FRONT_GAIN 0.35f 
...
*/
  ...
  if( micros()-timer > dt_us)
  {
    ...
    } else
    {
      ...
      {
        joyX_f = 3*alpha1*joyX + (1-3*alpha1)*joyX_f;
        if (abs(joyX_f)<0.05)
        {
          joyX_f = 0;
        }
...
*/
      ...
      //// Lecture du bouton de frein ///
       if(button_brake == 0 && abs(lateral_speed_ref)<0.2)
      {
        brake_flag = 0;
        brake_counter = 0;
      } else
      ...
      } else
      {
        ...
        brake_counter ++;
        if(brake_counter>BRAKE_LEVEL_MAX-1)
        {
          brake_counter = 0;
        }
        brake_flag = 1;
      ...
      lateral_speed_ref = (1+abs(front_speed_ref))*LAT_VS_FRONT_GAIN*joyX_f;

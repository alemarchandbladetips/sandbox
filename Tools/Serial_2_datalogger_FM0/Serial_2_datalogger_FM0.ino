...
void setup() {
  ...
  if(print_data){Serial.print("Initializing SD card...");}
  if (!SD.begin(chipSelect)) {
    if(print_data){Serial.println("Card failed, or not present");}
    // don't do anything more:
    return;
  }
  if(print_data){Serial.println("card initialized.");}
...
// button = 1 when not pressed and 0 when pressed
  ...
  if((state == 1) && ((millis()-click_time) > 1000) && (state2 == 0))
  {
    ...
    if(log_state == 0)
    {
      if(print_data){Serial.print("Opening file ... ");}
      filename = "log2s";
      filename = filename + long_num;
      filename = filename + ".txt";
      if(print_data){Serial.println(filename);}
      dataFile = SD.open(filename, FILE_WRITE);
      if(dataFile)
      {
        if(print_data){Serial.println("file openend");}
        log_state = 1;
        long_num++;
        log_start_time = millis();
      }else
      {
        if(print_data){Serial.println("Opening failed");}
      }
    } else
    {
      if(print_data){Serial.print("Closing file ... ");}
      dataFile.close();
      if(print_data){Serial.println("file closed");}
      log_state = 0;
    }
  }
  ...
  if(Serial1.available()>SERIAL1_DATA_SIZE+1)
  {
    ...
    if(x == 137)
    {
      ...
      if( serial1_buffer[SERIAL1_DATA_SIZE]==173 && (log_state == 1))
      {
        ...
        if(print_data){Serial.print(serial2_buffer[12]);Serial.print("\t");}
        ...
        if(print_data){Serial.print(serial2_buffer[23]);Serial.print("\t");}
        dataFile.print(serial2_buffer[23]);dataFile.print("\t");
        if(print_data){Serial.print(serial2_buffer[24]);Serial.print("\t");}
...
//        Serial.println(" ");
//      }
        if(print_timing){Serial.print("S1: ");Serial.println(micros()-timer_tmp);}
    } // else
//    {
...
//    }
  ...
  if(Serial2.available()>SERIAL2_DATA_SIZE+1)
  {
    ...
    if(x == 137)
    {
      ...
      if(print_timing){Serial.print("S2: ");Serial.println(micros()-timer_tmp);}
    }
  }

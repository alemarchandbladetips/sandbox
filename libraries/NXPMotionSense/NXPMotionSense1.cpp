...
bool NXPMotionSense1::begin()
{
	...
	while (!FXOS8700_begin()) {
		Serial.println("config1 error FXOS8700");
		delay(1000);
	}
	while (!FXAS21002_begin()) {
		Serial.println("config1 error FXAS21002");
		delay(1000);
	}
	while (!MPL3115_begin()) {
		Serial.println("config1 error MPL3115");
		delay(1000);
	}
...
static bool read_regs(uint8_t i2c, uint8_t addr, uint8_t *data, uint8_t num)
{
	...
	Wire1.requestFrom(i2c, num);
	if (Wire1.available() != num) return false;
	while (num > 0) {
		*data++ = Wire1.read();
		num--;
	}
...
bool NXPMotionSense1::FXOS8700_begin()
{
...
bool NXPMotionSense1::FXOS8700_read(int16_t *data)  // accel + mag
{
...
bool NXPMotionSense1::FXAS21002_begin()
{
...
bool NXPMotionSense1::FXAS21002_read(int16_t *data) // gyro
{
	static elapsedMicros usec_since;
...
bool NXPMotionSense1::MPL3115_begin() // pressure
{
...
//Serial.println("3");
	...
	if (!write_reg(i2c_addr, MPL3115_CTRL_REG1, 0x91)) return false;
	// enable events
...
bool NXPMotionSense1::MPL3115_read(int32_t *altitude, int16_t *temperature)
{
	static elapsedMicros usec_since;
	static int32_t usec_history=1000;
	const uint8_t i2c_addr=MPL3115_I2C_ADDR;
	uint8_t buf[6];
	...
	if (!write_reg(i2c_addr, MPL3115_CTRL_REG1, 0x93)) return false;
	
	//Serial.print(" 1 ");

	if (!read_regs(i2c_addr, MPL3115_STATUS, buf, 1)) return false;
	//Serial.print(" 2 ");
	if (buf[0] == 0) return false;
	//Serial.print(" 3 ");
...
bool NXPMotionSense1::writeCalibration(const void *data)
{
